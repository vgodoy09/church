...
/**
 * Servlet implementation class ControleIgreja
 */
@WebServlet("/ControleIgreja")
public class ControleIgreja extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public ControleIgreja() {
        super();
    }

	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(request.getParameter("btnNovo") != null) {
			request.getRequestDispatcher("/church/cadastroigreja.jsp").forward(request, response);
			...
			try {
				...
				request.getRequestDispatcher("/menu/atualizarigreja.jsp").forward(request, response);
			} catch (Exception e) {
				e.printStackTrace();
			}
...
//				request.getRequestDispatcher("ControlarListagemIgrejas").forward(request, response);
//			} catch (Exception e) {
//				e.printStackTrace();
//			}
